    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui informasi untuk akun {{ $user->nama_user }}.
                </p>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-dark">
            <form action="{{ route('pengguna.update', $user->id) }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <!-- Nama Lengkap -->
                    <div>
                        <label for="nama_user" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama
                            Lengkap</label>
                        <input type="text" id="nama_user" name="nama_user" value="{{ old('nama_user', $user->nama_user) }}"
                            required
                            class="h-11 w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-gray-500 dark:focus:border-brand-500 transition duration-150">
                    </div>
                    ...
                    <div>
                        <label for="username"
                            class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Username</label>
                    ...
                    <!-- Role -->
                    <div>
                        <label for="role" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Role
                            Akses</label>
                        @if($user->id === auth()->id())
                            ...
                            <p class="mt-1.5 text-xs text-amber-600 dark:text-amber-400">⚠️ Anda tidak dapat mengubah role untuk
                                akun Anda sendiri.</p>
                        @else
                            <div class="relative">
                    ...
                    <div>
                        <label for="password"
                            class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Password
                            (Opsional)</label>
