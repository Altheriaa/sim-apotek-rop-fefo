...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex gap-2">
            @if(request('tanggal_dari') && request('tanggal_sampai'))
                <a href="{{ route('laporan.obat-keluar.pdf', ['start_date' => request('tanggal_dari'), 'end_date' => request('tanggal_sampai')]) }}" target="_blank" class="inline-flex items-center gap-1.5 rounded-lg bg-error-500 px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-error-600">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-sm dark:border-gray-800 dark:bg-gray-dark">
        <form action="{{ route('laporan.obat-keluar') }}" method="GET" class="flex flex-col sm:flex-row items-end gap-4">
