                    ...
                    <div class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center flex-wrap">
                        ...
                        <div class="relative w-full sm:w-48">
                            <select name="status_ed" onchange="this.form.submit()"
                                ...
                                <option value="expired" {{ request('status_ed') === 'expired' ? 'selected' : '' }} class="dark:bg-gray-900">Kadaluwarsa</option>
                            </select>
                ...
                <table class="w-full min-w-[1050px] text-left">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
                            ...
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400">Tgl Masuk</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400">Obat & Kemasan</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400">Supplier</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400">No. Batch</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400">Tgl. ED</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 text-right">Harga Beli Satuan</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 text-right">Total Pembelian</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 text-center">Di Gudang</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 text-center">Di Rak</th>
                            <th class="px-4 py-3 text-sm font-medium text-gray-500 dark:text-gray-400 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($batches as $trx)
                            @php
                                $isi = max(1, (int) ($trx->obat->isi_per_kemasan ?? 1));
                                $modalPerJual = $isi > 0 ? ((float) $trx->harga_beli_satuan / $isi) : 0;
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-800 dark:text-white/90 whitespace-nowrap">
                                    {{ \Carbon\Carbon::parse($trx->tanggal_masuk)->format('d M Y') }}
                                </td>
                                <td class="px-4 py-4 text-sm">
                                    <div class="font-bold text-gray-800 dark:text-white/90">{{ $trx->obat->nama_obat ?? '-' }}</div>
                                    <div class="text-[11px] text-gray-400">
                                        {{ $trx->obat->kode_obat }}
                                        @if($isi > 1)
                                            - 1 {{ $trx->obat->satuan_beli }} = {{ $isi }} {{ $trx->obat->satuan_jual }}
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    <div class="font-medium text-gray-700 dark:text-gray-300">{{ $trx->supplier->nama_supplier ?? '-' }}</div>
                                    @if($trx->supplier && $trx->supplier->kontak)
                                        <div class="text-[11px] text-gray-400">{{ $trx->supplier->kontak }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-sm font-mono font-semibold text-brand-600 dark:text-brand-400 text-xs">
                                    {{ $trx->nomor_batch }}
                                </td>
                                <td class="px-4 py-4 text-sm">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $isExpired ? 'bg-error-100 text-error-700 dark:bg-error-900/30 dark:text-error-400' : ($isNear ? 'bg-warning-100 text-warning-700 dark:bg-warning-900/30 dark:text-warning-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300') }}">
                                        {{ $ed->format('d M Y') }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-sm text-right">
                                    <div class="font-bold text-gray-800 dark:text-white/90">
                                        Rp {{ number_format($trx->harga_beli_satuan, 0, ',', '.') }}
                                        <span class="text-xs font-normal text-gray-400">/ {{ $trx->obat->satuan_beli }}</span>
                                    </div>
                                    @if($isi > 1)
                                        <div class="text-[11px] font-medium text-brand-600 dark:text-brand-400">
                                            (Rp {{ number_format($modalPerJual, 0, ',', '.') }}/{{ $trx->obat->satuan_jual }})
                                        </div>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-sm font-bold text-gray-800 dark:text-white/90 text-right">
                                    Rp {{ number_format($trx->harga_beli, 0, ',', '.') }}
                                </td>
                                <td class="px-4 py-4 text-center text-sm font-semibold text-blue-600 dark:text-blue-400">
                                    {{ $trx->stok_gudang }} <span class="text-xs font-normal text-gray-400">{{ $trx->obat->satuan_beli ?? '' }}</span>
                                </td>
                                <td class="px-4 py-4 text-center text-sm font-semibold text-green-600 dark:text-green-400">
                                    {{ $trx->stok_rak }} <span class="text-xs font-normal text-gray-400">{{ $trx->obat->satuan_jual ?? '' }}</span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <a href="{{ route('obat-masuk.show', $trx->id) }}"
                                        class="inline-flex items-center justify-center gap-1.5 h-8 px-3 rounded-lg border border-gray-200 bg-white text-xs font-medium text-gray-700 hover:bg-gray-50 hover:text-brand-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 transition"
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="px-5 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
