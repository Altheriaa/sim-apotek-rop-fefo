...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Pencatatan penerimaan obat dari supplier dan pengaturan batch / ED.</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('obat-masuk.create') }}" class="inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">
                    ...
                    @forelse($batches as $trx)
                        <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/20">
                            ...
                            <td class="px-5 py-4 text-sm">
                                <span class="{{ \Carbon\Carbon::parse($trx->tanggal_kadaluwarsa)->isPast() ? 'text-error-500 font-medium' : 'text-gray-500 dark:text-gray-400' }}">
                                    {{ \Carbon\Carbon::parse($trx->tanggal_kadaluwarsa)->format('d M Y') }}
                                </span>
                            </td>
