            ...
            <div class="rounded-xl border border-brand-200 bg-brand-50/70 p-4 dark:border-brand-800/30 dark:bg-brand-900/10 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-brand-700 dark:text-brand-300">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="border-b border-gray-100 p-4 sm:p-5 dark:border-gray-800">
                <form action="{{ route('riwayat-penjualan.index') }}" method="GET"
                    ...
                    <div class="flex items-center gap-2">
                        <button type="submit" class="h-10 inline-flex items-center justify-center gap-1.5 rounded-lg bg-brand-500 px-4 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-500/30 transition">Filter</button>
                        @if(request()->hasAny(['search', 'tanggal_dari', 'tanggal_sampai']))
                            <a href="{{ route('riwayat-penjualan.index') }}" class="h-10 inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white px-3.5 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white transition" title="Reset Filter">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
